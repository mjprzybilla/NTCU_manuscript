...
Content:
```
└─ data_sim_initial_condition/: Directory containing images obtained from the experimental Tam Ctrl (4 days), which are used as initial conditions for the numerical simulations. 
└─ neutral_model/: Directory containing scripts to run simulations of the neutral model.
...
└─ non_neutral_model/: Directory containing scripts to run simulations of the non neutral model.
...
| └─ data/: Directory containing data used in Plot scripts
...
└─ non_neutral_model_w_driver_mutation/: Directory containing scripts to run simulations of non neutral model in which one clone has an advantage over neighbouring clones.
